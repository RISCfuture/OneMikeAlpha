...
class BulkInserter
  ...
  module Strategy
    ...
    class Base
      ...
      attr_reader :base, :records_inserted
    ...
    class FileBased < Base
      def _add(row)
        values = row.map do |value|
          case value
            ...
            when Time then value.strftime('%Y-%m-%d %H:%M:%S.%L')
            else value.to_s
          end
        end
      ...
      def sudo(query)
        Rails.application.operation_logger.system OneMikeAlpha::Configuration.psql.path,
                                                  '-c',
                                                  query,
    ...
    class FDW < FileBased
      ...
      def finish!
        ...
        sudo sql
        sudo "ALTER TABLE #{quoted_table_name} OWNER TO #{model.connection_db_config.configuration_hash[:username].inspect}"
      ...
      def client_encoding
        case encoding.to_s
          when 'Big5' then 'BIG5'
          when 'EUC-JIS-2004' then 'EUC-JIS_2004'
          when 'EUC-JP' then 'EUC_JP'
          when 'EUC-KR' then 'EUC_KR'
          when 'EUC-TW' then 'EUC_TW'
          when 'GB18030' then 'GB18030'
          when 'GBK' then 'GBK'
          when 'ISO-8859-5' then 'ISO_8859_5'
          when 'ISO-8859-6' then 'ISO_8859_6'
          when 'ISO-8859-7' then 'ISO_8859_7'
          when 'ISO-8859-8' then 'ISO_8859_8'
          when 'KOI8-R' then 'KOI8R'
          when 'KOI8-U' then 'KOI8U'
          when 'ISO-8859-1' then 'LATIN1'
          when 'ISO-8859-2' then 'LATIN2'
          when 'ISO-8859-3' then 'LATIN3'
          when 'ISO-8859-4' then 'LATIN4'
          when 'ISO-8859-9' then 'LATIN5'
          when 'ISO-8859-10' then 'LATIN6'
          when 'ISO-8859-13' then 'LATIN7'
          when 'ISO-8859-14' then 'LATIN8'
          when 'ISO-8859-15' then 'LATIN9'
          when 'ISO-8859-16' then 'LATIN10'
          when 'Emacs-Mule' then 'MULE_INTERNAL'
          when 'Shift_JIS' then 'SJIS'
          when 'UTF-8' then 'UTF8'
          when 'IBM866' then 'WIN866'
          when 'Windows-874' then 'WIN874'
          when 'Windows-1250' then 'WIN1250'
          when 'Windows-1251' then 'WIN1251'
          when 'Windows-1252' then 'WIN1252'
          when 'Windows-1253' then 'WIN1253'
          when 'Windows-1254' then 'WIN1254'
          when 'Windows-1255' then 'WIN1255'
          when 'Windows-1256' then 'WIN1256'
          when 'Windows-1257' then 'WIN1257'
          when 'Windows-1258' then 'WIN1258'
          else 'SQL_ASCII'
        end
      end
