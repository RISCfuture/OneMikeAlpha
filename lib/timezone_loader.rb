...
class TimezoneLoader
  ...
  def load_shapes_the_easy_way(tmpdir, shapes)
    ...
    db_options   = Timezone.connection_db_config
    psql_options = +''
    psql_options << '-U' << Shellwords.escape(db_options.configuration_hash[:username]) if db_options.configuration_hash[:username]
    psql_options << '-h' << Shellwords.escape(db_options.host) if db_options.host
    psql_options << '-p' << Shellwords.escape(db_options.port) if db_options.configuration_hash[:port]
    psql_options << ' ' << Shellwords.escape(db_options.database)
    env = db_options.configuration_hash[:password] ? "PGPASSWORD=#{Shellwords.escape db_options.configuration_hash[:password]}" : nil
