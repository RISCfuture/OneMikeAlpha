...
class TelemetryQueryBuilder
  attr_reader :aircraft, :time_period, :interval, :parameter_paths
  ...
  class Relation
    ...
    def add_select(parameter, name)
      @selects[name] = @model.arel_table[parameter]
    end
      ...
      case part
        when :x
          function = Arel::Nodes::NamedFunction.new('ST_X',
                                                    [ah.cast(@model.arel_table[parameter], 'geometry')])
