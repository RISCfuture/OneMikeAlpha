...
class Flight < ApplicationRecord
  ...
  scope :covering_range, ->(start_time, end_time) {
    where(arel_table[:recording_start_time].not_eq(nil).
        and(ah.cover(
                ah.tsrange(arel_table[:recording_start_time], arel_table[:recording_end_time]),
                ah.tsrange(Arel::Nodes.build_quoted(start_time.utc, arel_table[:recording_start_time]),
                           Arel::Nodes.build_quoted(end_time.utc, arel_table[:recording_end_time]))
)))
  }

  MIN_SIGNIFICANT_TIME            = 0.1.hours
  MIN_SIGNIFICANT_ALTITUDE_CHANGE = Unit.new('50 m')
  ...
  scope :with_telemetry, ->(model=Telemetry) {
    telemetry_joins = arel_table.join(model.arel_table).on(
        ah.contains(ah.tsrange(arel_table[:recording_start_time], arel_table[:recording_end_time]),
                    model.arel_table[:time]).and(model.arel_table[:aircraft_id].eq(arel_table[:aircraft_id]))
    )
    where(
        arel_table[:recording_start_time].not_eq(nil).
            and(arel_table[:recording_end_time].not_eq(nil))
    ).joins(telemetry_joins.join_sources)
  }

  scope :with_distance, -> {
    select(ah.st_length(ah.cast(ah.st_make_line(
        ah.st_start_point(ah.cast(arel_table[:track], 'geometry')),
        ah.st_end_point(ah.cast(arel_table[:track], 'geometry'))
    ), 'geography')).as('distance'))
  }
  scope :with_distance_flown, -> {
    select(ah.st_length(arel_table[:track]).as('distance_flown'))
  }

  scope :_significant_height, -> {
    with_telemetry(Telemetry::PositionSensor).
        group(arel_table[:id]).
        having(ah.subtract(Telemetry::PositionSensor.arel_table[:height_agl].maximum,
                           Telemetry::PositionSensor.arel_table[:height_agl].minimum).
            gt(50))
  }
  ...
  scope :_significant_duration, -> {
    where(arel_table[:departure_time].not_eq(nil).
        and(arel_table[:arrival_time].not_eq(nil)).
        and(ah.subtract(arel_table[:arrival_time], arel_table[:departure_time]).
            gt(Arel.sql("INTERVAL '0.1 HOUR'"))))
  }
  scope :_insignificant_duration, -> {
    where(arel_table[:departure_time].eq(nil).
        or(arel_table[:arrival_time].eq(nil)).
        or(ah.subtract(arel_table[:arrival_time], arel_table[:departure_time]).
  ...
  def bounds
    query = self.class.select(
        ah.st_extent(ah.cast(self.class.arel_table[:track], 'geometry')).as('extent')
  ...
  def start_comes_before_end
    if recording_start_time && recording_end_time && recording_start_time > recording_end_time
      errors.add(:recording_end_time, :comes_before_start)
    end
    if departure_time && arrival_time && departure_time > arrival_time
      errors.add(:arrival_time, :comes_before_start)
    end
    if takeoff_time && landing_time && takeoff_time > landing_time
      errors.add(:landing_time, :comes_before_start)
    end
  end
  ...
  def recalculate_geolocation
    position = Telemetry::Position.arel_table[:position]
  ...
  def airport_near_telemetry(telemetry)
    airport = Airport.where(Airport.arel_table[:fadds_site_number].not_eq(nil)).closest_to(telemetry.position).first
    if airport.nil? || Airport.distance_between(telemetry.position, airport).first.distance > MIN_AIRPORT_DISTANCE
      airport = Airport.where(Airport.arel_table[:icao_number].not_eq(nil)).closest_to(telemetry.position).first
  ...
  def recalculate_departure_arrival_times
    ground_speed = Telemetry::PositionSensor.arel_table[:ground_speed]
  ...
  def recalculate_takeoff_landing_times
    telemetry_buckets = telemetry(Telemetry::PositionSensor, Telemetry::PitotStaticSystem, time: recording_period).
        time_bucket(5.seconds,
                    Telemetry::PositionSensor.arel_table[:ground_speed],
                    Telemetry::PitotStaticSystem.arel_table[:vertical_speed])

    avg_ground_speed   = Telemetry::PositionSensor.arel_table[:ground_speed].average
    avg_vertical_speed = Telemetry::PitotStaticSystem.arel_table[:vertical_speed].average
