module TelemetryMixin
  ...
  included do
    scope :join_telemetry, ->(model) do
      join_clause = arel_table.join(model.arel_table, Arel::Nodes::OuterJoin).
          on(model.arel_table[:aircraft_id].eq(arel_table[:aircraft_id]).
              and(arel_table[:time].eq(model.arel_table[:time])))
  ...
  class << self
    ...
    def average_arel_attribute(col, model=nil)
      model ||= col.relation.instance_variable_get(:@klass)
