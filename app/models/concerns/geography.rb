  ...
  module ClassMethods
    ...
    def define_point_scopes(column)
      ...
      scope :closest_to, ->(location, parameter_name: 'distance') {
        ...
        select(
            Arel.star,
            ah.distance(arel_attribute(column), ah.geo_from_text(location)).as(parameter_name)
    ...
    def define_polygon_scopes(column)
      scope :containing, ->(point) {
        where(ah.st_covers(arel_attribute(column), ah.geo_from_text(point)))
      }
    end
  end
