...
class Upload < ApplicationRecord
  ...
  def process_inline!
    ImportWorkflow.perform_inline(self) { |workflow| update_attribute :workflow_uuid, workflow.id }
  end
  ...
  class FileImporter
    ...
    def download
      download_blob_to_tempfile { |file| yield file }
    end
    ...
    def id() file.id end
    # @private
    def self.find(id) FileImporter.new(ActiveStorage::Attachment.find(id)) end
  end
