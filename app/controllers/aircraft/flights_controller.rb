  ...
  def index
    ...
    @flights = sort do |column|
      column.time(default: true, default_dir: :desc) do |dir|
        ...
        case dir
          when :asc
            flights = flights.where(Flight.arel_attribute(:departure_time).gt(departure_time)) if departure_time
            flights = flights.order(departure_time: :asc)
          when :desc
            flights = flights.where(Flight.arel_attribute(:departure_time).lt(departure_time)) if departure_time
