...
export class UnitSystem {
  ...
  static base = new UnitSystem()

  constructor(definitions?: {[dimension: string]: string}) {
    this.units = new Map()
    forOwn(definitions, (value, key) => this.units.set(key, value))
  }

  static defineBase(definitions: {[dimension: string]: string}) {
    ...
    if (this[dimension] === 'gee') {
      const converter = new class {
        ...
        convert(value: number | number[]): number | number[] {
          if (isArray(value)) {
            const result = func(<number[]>value)
