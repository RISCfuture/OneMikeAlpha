...
<script lang="ts">
  ...
  export default class Chart extends Vue {
    $refs!: {
      chart: HTMLElement & {chart: Highcharts.Chart}
    }
    ...
    private get plotBands(): Highcharts.XAxisPlotBandsOptions[] {
      ...
      const normalizedParameters = uniq(this.info.parameters.map(p => this.normalizeParameter(p)))
    ...
    private broadcastScrub(e: Highcharts.PointerEventObject) {
      ...
      const currentPoint = chart.series[0].searchPoint(event, true)
      if (!currentPoint) return
      highlightPoint(currentPoint, e)

      ScrubberBus.$emit('scrub', e, currentPoint)
    }

    private slaveScrubber(event: Highcharts.PointerEventObject, currentPoint: Highcharts.TooltipPositionerPointObject) {
      ...
      let e2: ChartEvent = assign({}, event, {
        chartX: currentPoint.plotX + chart.plotLeft,
        chartY: currentPoint.plotY + chart.plotTop
      })
      highlightPoint(point, e2)
    }
    ...
    private isEstimated(param: string): boolean {
      ...
      return includes(estimated, this.normalizeParameter(param))
    }
